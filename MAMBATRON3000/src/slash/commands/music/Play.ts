...
export default class Play extends Command {
  ...
  private async play(
    ...
    // Get the song info
    let songInfo: ytdl.videoInfo = null as any;; 
    try {
  ...
  private async connectToChannel(
    ...
    try {
      ...
      connection.on<"stateChange">("stateChange", async (_, newState) => {
  ...
  private addSongToQueue(
    ...
    let musicQueue: IServerMusicQueue | undefined  = this.client.musicQueue.get(guild.id);
    if (musicQueue === undefined) {
      ...
      this.client.musicQueue.set(guild.id, musicQueue as IServerMusicQueue);
    }

    musicQueue?.songs.push(song);
    return musicQueue as IServerMusicQueue;
  }
