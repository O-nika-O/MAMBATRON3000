...
export const WelcomeMessagePage = () => {
  ...
  const saveWelcomeChannel = async (
    ...
    // handleFocus(input);
    try {
      const res = await updateWelcomeChannelId(guildId, selectedChannel || "" , message || "");
  ...
  const resetWelcome = async (
    ...
    // handleFocus(input);
    try {
      const res = await updateWelcomeChannelId(guildId, selectedChannel || "", "Welcome to the server");      
      setLoadingUpdate(true);
      console.log(res);
    } catch (err) {
      console.log(err);
  ...
  return (
    <Page>
      <Container>
        ...
        {channels && config && !loading ? (
          <div>
            ...
            >
              ...
              <Button variant="primary" onClick={saveWelcomeChannel}>
                Save
              </Button>
            </Flex>
          </div>
        ...
        item ? <animated.h3 style={style}>Welcome channel settings updated succesfully</animated.h3> : ''
        )}
      </Container>
    </Page>
  );
